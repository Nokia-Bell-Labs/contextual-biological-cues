...
class Feature_Matching:
    ...
    def run(self):
        self.min_max_scale_data() # for both train and test
    ...
    def min_max_scale_data(self):
        for df in [self.train_df, self.test_df]:
            scaler = MinMaxScaler()
            df.loc[:, self.feature_columns] = scaler.fit_transform(df.loc[:, self.feature_columns])

        # Check if the scaling is correct
        tolerance = 1e-9
        for df in [self.train_df, self.test_df]:
            max_vals = df[self.feature_columns].max()
            min_vals = df[self.feature_columns].min()
    ...
    def train_matching(self):
        ...
        for batch in train_loader:
            ...
            pos_dist, neg_dist = self.compute_feat_distance_train(data)
    ...
    def compute_feat_distance_train(self, data):
        """Computes feature distances for training set."""
    ...
    def compute_feat_distance_test(self, data):
        """Computes feature distances for test set with two negative sets (sameuser and diffuser)."""
        anchor = data['anchor'].clone().cpu().numpy()
        positive = data['positive'].clone().cpu().numpy()
        ...
        res_neg_dist = np.concatenate(res_neg_dists, axis=0)  # (2*bs, num_feat_cols)
    ...
    def compute_distances(self, anchor, positive, negatives_list):
        ...
        for col_idx in range(num_feat_cols): # traverse over feature columns
    ...
    def eval_matching(self, trained_model):
        ...
        for batch in test_loader:
            ...
            pos_dist, neg_dist = self.compute_feat_distance_test(data)
            pos_dist_list.extend(pos_dist)
            neg_dist_list.extend(neg_dist)

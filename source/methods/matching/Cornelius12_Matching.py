...
class Cornelius12_Matching:
    ...
    def train_matching(self):
        ...
        for batch in train_loader:
            ...
            pos_sim, neg_sim = self.compute_coh_similarities_train(data)
            pos_sim_list.extend(pos_sim)
            neg_sim_list.extend(neg_sim)

        # Train SVM classifier
        # Use rbf kernel according to paper
        svm_model = SVC(kernel='rbf', class_weight='balanced') # adjust class_weight for imbalanced dataset
        # Create X_train and y_train from pos_sim_list and neg_sim_list
        X_train = np.array(pos_sim_list + neg_sim_list)
        ...
        et_match = time.time()
        self.logger.info(f"[{self.task}-{self.method}] Training time: {et_match - st_match:.2f} seconds")

        return svm_model


    def compute_coh_similarities_train(self, data):
        anchor = data['anchor'].clone().cpu().numpy()
        positive = data['positive'].clone().cpu().numpy()
        negative = data['negative'].clone().cpu().numpy()

        res_pos_sim, res_neg_sim = self._compute_coh_similarities(anchor, positive, [negative])
    ...
    def compute_coh_similarities_test(self, data):
        anchor = data['anchor'].clone().cpu().numpy()
        positive = data['positive'].clone().cpu().numpy()
        neg_sameuser = data['neg_sameuser'].clone().cpu().numpy()
        neg_diffuser = data['neg_diffuser'].clone().cpu().numpy()

        res_pos_dist, res_neg_dists = self._compute_coh_similarities(anchor, positive, [neg_sameuser, neg_diffuser])
    ...
    def _compute_coh_similarities(self, anchor, positive, negatives_list):
        """Helper function to compute distances between anchor, positive, and multiple negative sets."""
        pos_sim_list = []
        neg_sim_lists = [[] for _ in range(len(negatives_list))]

        for i in range(anchor.shape[0]): # traversing across the batch size (accessing each element)

            # Compute pos similarities
            pos_sim = compute_coherence_features(anchor[i], positive[i], self.fs, self.task) # computed for single pair
            pos_sim_list.append(pos_sim)

            # Compute for negative similarities
            for j, negative in enumerate(negatives_list):
                neg_sim = compute_coherence_features(anchor[i], negative[i], self.fs, self.task)
                neg_sim_lists[j].append(neg_sim)


        res_pos_sim = np.stack(pos_sim_list, axis=0)  # (bs, num_feat_cols)
        res_neg_sims = [np.stack(neg_sim_lists, axis=0) for neg_sim_lists in neg_sim_lists]  # List of (bs, num_feat_cols)

        return res_pos_sim, res_neg_sims



    def eval_matching(self, trained_svm_model):
        ...
        for batch in test_loader:
            ...
            pos_sim, neg_sim = self.compute_coh_similarities_test(data)
            pos_sim_list.extend(pos_sim)
            neg_sim_list.extend(neg_sim)

        # Create X_test and y_test from pos_sim_list and neg_sim_list
        X_test = np.array(pos_sim_list + neg_sim_list)
